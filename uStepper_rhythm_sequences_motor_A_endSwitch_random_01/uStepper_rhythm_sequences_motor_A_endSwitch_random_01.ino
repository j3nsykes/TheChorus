...
// End switch pin
#define END_SWITCH_PIN 8   // End switch connected to D8
#define DEBUG_LED_PIN 13   // Built-in LED for visual debugging
#define MOVE_DISTANCE 2.0  // Degrees to move in each step
#define HOME_OFFSET 200.0  // Degrees to move up after finding switch

float targetHoldPosition = 0.0;
...
// Keep existing pattern definitions, but make a copy that can be modified
byte dynamicPatterns[8][16];  // 8 patterns with up to 16 beats each

// Initialize dynamic patterns from the static ones in setup()
void initializeDynamicPatterns() {
  // Copy all static patterns to dynamic ones
  for (int i = 0; i < sizeof(patterns) / sizeof(patterns[0]); i++) {
    for (int j = 0; j < 16; j++) {
      dynamicPatterns[i][j] = patterns[i][j];
      if (patterns[i][j] == BEAT_TERMINATOR) break;
    }
  }
}
...
void setup() {
  ...
  // Initialise dynamic patterns
  initializeDynamicPatterns();

  // Initial random value for pattern 0
  updateRandomPattern();
  //comment in and comment line above out for multiple patterns
...
bool performAutoHoming() {
  ...
  bool switchPressed = (digitalRead(END_SWITCH_PIN) == LOW);
  Serial.print("Initial switch state: ");
  Serial.println(switchPressed ? "PRESSED" : "NOT PRESSED");

  float initialPosition = stepper.encoder.getAngle();  // Track initial position

  // SCENARIO 1: Switch is already activated at startup
  if (switchPressed) {
    Serial.println("End switch is already activated at startup.");
    Serial.println("SWITCH IS PRESSED - MOVING UP");


    // Move up in increments of 10 degrees
    for (int i = 0; i < 10; i++) {
      Serial.print("Move up step ");
      Serial.print(i + 1);
      Serial.print("/10 - Angle: ");
      Serial.println(stepper.encoder.getAngle());

      stepper.moveAngle(2.0);
      delay(300);  // Give more time between moves

      // Check for timeout
      if (millis() - startTime > safetyTimeout) {
        Serial.println("Homing timeout during upward movement.");
        return false;
      }
    }

    // Check current position and switch state
    Serial.print("Position after upward movement: ");
    Serial.println(stepper.encoder.getAngle());
    Serial.print("Switch state: ");
    Serial.println(digitalRead(END_SWITCH_PIN) == LOW ? "PRESSED" : "NOT PRESSED");
  }
  // SCENARIO 2: Switch not activated at startup
  else {
    Serial.println("End switch is not activated at startup.");
    Serial.println("Moving DOWN to find switch...");

    // Step 1: Move down until switch is pressed
    int moves = 0;
    while (digitalRead(END_SWITCH_PIN) == HIGH && moves < 100) {
      stepper.moveAngle(-MOVE_DISTANCE);
      delay(100);
      moves++;
    ...
    // Step 2: Check if we found the switch
    if (digitalRead(END_SWITCH_PIN) == HIGH) {
      Serial.println("ERROR: Could not find switch!");
      return false;  // Exit if we couldn't find the switch
    }

    Serial.println("Switch found. Moving up...");

    // Step 3: Move up in increments
    for (int i = 0; i < 10; i++) {
      Serial.print("Move up step ");
      Serial.print(i + 1);
      Serial.print("/10 - Angle: ");
  ...
  // Now we've moved away from the switch in both scenarios

  // Force a complete stop of the motor
  stepper.stop();
  delay(500);  // Give time for the stop to take effect

  // Get the final position after all movements
  ...
  // Set the hold position for the main loop (from the simple code)
  // Set the hold position for the main loop
  targetHoldPosition = finalPosition;
  holdingPosition = true;

  // IMPORTANT: Update the MovingMotor startPos to the new zero position
  // This was missing in the original code
  motor.setStartPosition();

  Serial.print("Homing complete. Initial position: ");
...
void startQuickTest() {
  ...
  // If starting with pattern 0, update it with a random beat
  if (activePattern == 0) {
    updateRandomPattern();
  }
...
void checkQuickTestPattern() {
  ...
  if (currentTime - patternStartTime >= QUICK_TEST_PATTERN_TIME) {
    ...
    // Update pattern 0 with a random beat if it's selected
    if (activePattern == 0) {
      updateRandomPattern();
    }
...
bool isInPattern(int beat, int patternIndex) {
  ...
  // Use dynamic patterns instead of static ones
  const byte* pattern = dynamicPatterns[patternIndex];
  ...
  for (int i = 0; i < sizeof(timerData) / sizeof(timerData[0]); i++) {
    ...
    if (now >= startTime && now < endTime) {
      ...
      if (activePattern != timerPattern) {
        ...
        // If activating pattern 0, update with random beat
        if (activePattern == 0) {
          updateRandomPattern();
        }
  ...
  switch (cmd) {
    ...
    default:
      ...
      if (cmd >= '1' && cmd <= '9' && manualRun) {
        ...
        // If pattern 0 is selected, update it with a new random beat
        if (activePattern == 0) {
          updateRandomPattern();
        }
...
void printPattern(int patternIndex) {
  if (patternIndex < 0 || patternIndex >= sizeof(dynamicPatterns) / sizeof(dynamicPatterns[0])) {
