...
const byte quickTestPatterns[] = {
  0,   // Pattern 0 all together
  1,   // Pattern 1 up down
  3,   // Pattern 3 W rhythm
  5,   // Pattern 5 random
  9,   // Pattern 9 syncopate
  11,  // Pattern 11 single beat
  5,   // Pattern 5 random again
  13   // Pattern 13 single beat
};
...
// Perform automatic homing using the end switch
bool performAutoHoming() {
  // Safety timeout (3 minutes)
  unsigned long startTime = millis();
  unsigned long safetyTimeout = 180000; // 3 minutes
  
  // Check if end switch is already activated
  ...
  while (digitalRead(END_SWITCH_PIN) == HIGH) {
    ...
    stepper.moveAngle(-HOMING_SPEED);
  ...
  // Save this position as zero reference
  float switchPosition = stepper.encoder.getAngle();
  
  // Move away from the switch by HOME_OFFSET degrees
  Serial.print("Moving away from end switch by ");
